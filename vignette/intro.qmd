...
mkdatasetfiles(n_files = 1, size_mb = 25, prefix = "large_", dir = "data/large") -> large_file
mkdatasetfiles(n_files = 3, size_mb = 1,  prefix = "small_", dir = "data/small") -> small_files
mkdatasetfiles(n_files = 50, size_mb = 3,  prefix = "individual_", dir = "data/individual") -> individual_files
```
...
**Next up**: [Internals](intro-internals.html) — explore the `.dvs/` folder layout, meta files, and how content-addressed blob storage works under the hood. To try the same operations from the command line instead, see [CLI](intro-cli.html).
