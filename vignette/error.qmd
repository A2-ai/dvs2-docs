...
You're inside the project but ask dvs to track a file that lives somewhere
else on disk — by absolute path, or by a relative path that escapes the
project root. Both are rejected with a per-file error.
...
# Absolute path to a file outside the project
dvs_add(paths = outside_file)
```
...
Unlike `add`, querying status for an absolute path outside the project does
**not** raise a path-outside-project error — the path simply doesn't match
anything tracked, so you get the empty-set response instead. The same is
true for `get`.
...
dvs_status(paths = outside_status)
```
